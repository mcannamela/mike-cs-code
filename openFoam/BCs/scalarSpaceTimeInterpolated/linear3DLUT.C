...
class linear_three_D_LUT {
	...
	public:
		...
		double idx2(double t){
			double raw = (size2()-1)*(t-t2Min)/(t2Max-t2Min);
			int i = floor(raw);
			double f = raw-i;
			return i%size2()+f;
		}
...
linear_three_D_LUT::linear_three_D_LUT(const linear_three_D_LUT& other){
	//std::cout << "copying from other, who has t2 bounds: " << other.t2Min<<" "<<other.t2Max<<"\n";
	//std::cout <<"other's fname is "<< other.fname<<"\n";
...
linear_three_D_LUT::linear_three_D_LUT(std::string fname){
	...
	t2Max = getNum();
	n2    = getNum();
	//std::cout << "t2 bounds are " << t2Min<<" "<<t2Max<<"\n";
...
double linear_three_D_LUT::operator()(double t0, double t1, double t2){
			...
			/*std::cout << "interpolating...initialized, t's = "
			<< t0 << "  "
			<< t1 << "  "
			<< t2 << "  "
			<<"\nsizes: "
			...
			kk = idx2(t2);
			//if (t2Min<.008){
			//std::cout << "t2 bounds are " << t2Min<<" "<<t2Max<<", t2 is "<<t2<<"\n";}
			...
			x001 = X[k_plus][j_][i_];
			x011 = X[k_plus][j_+1][i_];
			x101 = X[k_plus][j_][i_+1];
			x111 = X[k_plus][j_+1][i_+1];
